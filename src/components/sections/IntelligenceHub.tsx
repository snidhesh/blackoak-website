...
import TileSnapshotSlider from '@/components/ui/TileSnapshotSlider';
...
// Real captures of each product feature, one per bullet, so a locked tile shows
// what subscribing opens. 2:1 PNGs under public/images/intelligence-hub, refreshed
// by hand when the products' look changes.
const FRAMES: Record<TileKey, { prefix: string; keys: string[] }> = {
  marketIntelligence: {
    prefix: 'mi',
    keys: ['chart', 'calculator', 'simulator'],
  },
            ...
            {TILES.map((key, index) => {
              ...
              return (
                <AnimateOnScroll key={key} delay={index * 0.1}>
                  ...
                  >
                    <TileSnapshotSlider
                      ariaLabel={t('snapshots.ariaLabel', {
                        product: t(`tiles.${key}.title`),
                      })}
                      prevLabel={t('snapshots.prev')}
                      nextLabel={t('snapshots.next')}
                      goToLabel={(n) => t('snapshots.goTo', { n })}
                      frames={FRAMES[key].keys.map((frame) => ({
                        src: `/images/intelligence-hub/${FRAMES[key].prefix}-${frame}.png`,
                        label: t(`tiles.${key}.frames.${frame}.label`),
                        alt: t(`tiles.${key}.frames.${frame}.alt`),
                      }))}
                    />
