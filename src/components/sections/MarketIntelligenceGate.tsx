...
import { cn, inertProps } from '@/lib/utils';
...
type SignupBody = NewsletterFormData & { locale: string; utm?: Record<string, string> };
...
function hasSubscribed(): boolean {
  try {
    ...
    // Storage blocked (private mode, site data disabled): treat as not subscribed.
    return false;
  }
}
...
export default function MarketIntelligenceGate({ children }: MarketIntelligenceGateProps) {
  ...
  const requestCode = async (body: SignupBody): Promise<boolean> => {
    ...
    const next: Pending = { challenge: result.challenge, expiresAt: result.expiresAt, body };
    savePending(next);
    setPending(next);
    setCode('');
    setCodeError(null);
  ...
  const onSubmitDetails = async (data: NewsletterFormData) => {
    ...
    setNotice(null);
    try {
      const body: SignupBody = { ...data, locale };
      const utm = readUtmFromLocation();
      if (utm) body.utm = utm;
      await requestCode(body);
    } catch {
      setError(tf('errorGeneric'));
    } finally {
      setSubmitting(false);
  ...
  // address unlocks the page, and only then is the team notified (server-side).
  const onVerify = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (err?.field === 'code') {
          setCodeError(
            err.code === NEWSLETTER_ERROR_CODES.codeExpired ? t('codeExpired') : t('codeInvalid')
          );
        } else {
      ...
      window.dataLayer?.push({ event: 'market_intelligence_subscribe' });
      setUnlocked(true);
      ...
      {locked && (
        <section className="mi-gate-panel bg-black text-white pb-20 md:pb-28">
          <div className="container-wide">
            ...
            <div id={panelId} hidden={!formOpen} className="mx-auto mt-10 max-w-xl bg-white p-6 text-black sm:p-10">
              ...
              {step === 'details' && (
                <>
                  <p className="mt-3 text-[15px] leading-[1.6] text-[#5F6368]">{t('body')}</p>
              ...
              {step === 'code' && pending && (
                <>
                  <p className="mt-3 text-[15px] leading-[1.6] text-[#5F6368]">
